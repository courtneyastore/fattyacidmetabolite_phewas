...
for (disease in disease_lst){
  ...
  output_file_name = paste0("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/PRSO3FA_BMI_WHR_PrevPerc_Batch/PRSO3FA_BMI_WHR_",disease_name_file,".pdf")
  ...
  plt <- ggplot(final_df, aes(x=PRS_percentile, y=Prev*100,col = model,fill=model, group = model)) + geom_point(alpha=0.3) + 
    ...
    xlab("Percentile of PGS-Omega-3 fatty acids") + ylab(disease_name_prev_label) +
