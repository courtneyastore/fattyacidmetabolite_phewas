...
disease_status_df <- disease_status_df[c('ID','Status_416')]
...
merge_df <- merge_df[merge_df$Status_416 != "Excluded", ]
merge_df$disease_status <- ifelse(merge_df$Status_416 == "Case", 1, 0)
...
ggplot(final_df, aes(x=PRS_percentile, y=Prev*100,col = model,fill=model, group = model)) + geom_point(alpha=0.3) + 
  ...
  xlab("Percentile of PGS-Omega-3 fatty acids") + ylab("Prevalence of Cardiomegaly (%)") +
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/Figure6/V2_O3FA_Cardiomegaly_BMI_WHR_PGSPrev_plt.pdf",height = 8,width = 8,dpi = 300,limitsize = FALSE)
