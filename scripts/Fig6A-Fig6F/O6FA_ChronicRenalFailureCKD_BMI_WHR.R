...
ggplot(final_df, aes(x=PRS_percentile, y=Prev*100,col = model,fill=model, group = model)) + geom_point(alpha=0.3) + 
  ...
  xlab("Percentile of PGS-Omega-6 fatty acids") + ylab("Prevalence of Chronic renal failure [CKD] (%)") +
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/Figure6/V2_O6FA_ChronicRenalFailureCKD_BMI_WHR_PGSPrev_plt.pdf",height = 8,width = 8,dpi = 300,limitsize = FALSE)
