...
nrow(merge_df[(merge_df$whr_departure > 1) | (merge_df$bmi_departure < -1), ])
nrow(merge_df[(merge_df$whr_departure < -2) | (merge_df$whr_departure > 2) | (merge_df$bmi_departure > 2) | (merge_df$bmi_departure < -2), ])
nrow(merge_df[(merge_df$whr_departure < -1) | (merge_df$whr_departure > 1) | (merge_df$bmi_departure > 1) | (merge_df$bmi_departure < -1), ])

merge_df$metabolite <- "O3FA"
merge_df <- merge_df[c('metabolite','disease','category','whr_departure','bmi_departure')]
#write.table(merge_df,file="/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/tables/ForGreg_O3FA_WHR_BMI_departures.tsv",quote=FALSE,sep="\t",row.names=FALSE,col.names = TRUE)
...
ggplot(merge_df, aes(y=whr_departure,x=bmi_departure,color=category)) +
  ...
  labs(x = "BMI delta departure",y="WHR delta departure", title = "Omega-3 fatty acids") + 
  ylim(-5,5) + xlim(-5,5) + 
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/V3_O3FA_BMI_DeltaDeparture_WHR_DeltaDeparture_Scatter_plt_Suggestive.pdf",height = 15,width = 15,dpi = 300,limitsize = FALSE)
...
o3fa_bmi_whr_departure_plt <- ggplot(merge_df, aes(reorder(disease,-ratio), ratio,color=category)) + 
  scale_color_manual(name="Disease group",values = disease_colors) +
  geom_point(size=5) + theme_bw() + geom_hline(yintercept=0) + coord_cartesian(ylim = c(-20, 20)) + 
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/O3FA_BMI_DeltaDeparture_WHR_DeltaDeparture_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
...
o3fa_bmi_whr_departure_plt <- ggplot(merge_df, aes(reorder(disease,-ratio), ratio,color=category)) + 
  ...
  theme(axis.text.x = element_text(size = 20,angle = 45, hjust=1), text = element_text(family = "Helvetica",size=20),legend.title=element_text(size=20),legend.text=element_text(size=20),strip.text = element_text(size=20),legend.position="bottom",axis.text.y = element_text(size = 20), axis.title = element_text(size = 20))
o3fa_bmi_whr_departure_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/O3FA_BMI_DeltaDeparture_WHR_DeltaDeparture_Suggestive_all_points.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
