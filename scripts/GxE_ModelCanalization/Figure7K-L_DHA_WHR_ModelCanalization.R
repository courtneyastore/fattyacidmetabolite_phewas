...
o3fa_bmi_observed_df <- as.data.frame(fread("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/processing_files/ModelCanalization_BMIWHR/ObservedDelta_DHAPGS_WHR_PrevPerc.tsv"))
o3fa_bmi_expected_df <- as.data.frame(fread("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/processing_files/ModelCanalization_BMIWHR/AvgSd_ExpectedDelta_DHAPGS_WHR_PrevPerc.tsv"))
...
o3fa_whr_plt <- ggplot(o3fa_bmi_df, aes(reorder(disease,-delta), delta, fill = label)) + 
  ...
  labs(x="Disease",y="Delta",title="K. Docosahexaenoic acid and WHR") +
...
o3fa_whr_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization/V2Fig7K_DHA_WHR_ObservedExpectedDelta_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
...
o3fa_whr_departure_plt <- ggplot(o3fa_bmi_departure_df, aes(reorder(disease,-departure), departure,color=category)) + 
  ...
  labs(x="Disease",y="Delta departure",title="L. Docosahexaenoic acid and WHR (Departure)") +
  theme(axis.text.x = element_text(size = 20,angle = 45, hjust=1), text = element_text(family = "Helvetica",size=20),legend.title=element_text(size=20),legend.text=element_text(size=20),strip.text = element_text(size=20),legend.position="bottom",axis.text.y = element_text(size = 20), axis.title = element_text(size = 20))
o3fa_whr_departure_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization/V2Fig7L_DHA_WHR_DeltaDeparture_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
