...
o3fa_bmi_plt <- ggplot(o3fa_bmi_df, aes(reorder(disease,-delta), delta, fill = label)) + 
  scale_fill_manual(name="Delta type",values = colors) +
  geom_bar(stat="identity", position = "dodge") + theme_bw() + 
  labs(x="Disease",y="Delta",title="A. Omega-3 fatty acids and BMI") +
...
o3fa_bmi_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization/V2Fig7A_O3FA_BMI_ObservedExpectedDelta_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
...
o3fa_bmi_departure_plt <- ggplot(o3fa_bmi_departure_df, aes(reorder(disease,-departure), departure,color=category)) + 
  scale_color_manual(name="Disease group",values = disease_colors) +
  geom_point(size=5) + theme_bw() + geom_hline(yintercept=0) + 
  labs(x="Disease",y="Delta departure",title="B. Omega-3 fatty acids and BMI (Departure)") +
  theme(axis.text.x = element_text(size = 20,angle = 45, hjust=1), text = element_text(family = "Helvetica",size=20),legend.title=element_text(size=20),legend.text=element_text(size=20),strip.text = element_text(size=20),legend.position="bottom",axis.text.y = element_text(size = 20), axis.title = element_text(size = 20))
o3fa_bmi_departure_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization/V2Fig7B_O3FA_BMI_DeltaDeparture_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
