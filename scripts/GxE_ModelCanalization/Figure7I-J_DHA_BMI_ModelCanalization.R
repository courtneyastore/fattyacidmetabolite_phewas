...
o3fa_whr_plt <- ggplot(o3fa_bmi_df, aes(reorder(disease,-delta), delta, fill = label)) + 
  ...
  labs(x="Disease",y="Delta",title="I. Docosahexaenoic acid and BMI") +
...
o3fa_whr_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization/V2Fig7I_DHA_BMI_ObservedExpectedDelta_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
...
o3fa_whr_departure_plt <- ggplot(o3fa_bmi_departure_df, aes(reorder(disease,-departure), departure,color=category)) + 
  ...
  labs(x="Disease",y="Delta departure",title="J. Docosahexaenoic acid and BMI (Departure)") +
  theme(axis.text.x = element_text(size = 20,angle = 45, hjust=1), text = element_text(family = "Helvetica",size=20),legend.title=element_text(size=20),legend.text=element_text(size=20),strip.text = element_text(size=20),legend.position="bottom",axis.text.y = element_text(size = 20), axis.title = element_text(size = 20))
o3fa_whr_departure_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization/V2Fig7J_DHA_BMI_DeltaDeparture_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
