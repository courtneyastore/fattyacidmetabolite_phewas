...
o3fa_bmi_observed_df <- as.data.frame(fread("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/processing_files/ModelCanalization_BMIWHR/ObservedDelta_DHAPGS_BMI_PrevPerc.tsv"))
o3fa_bmi_expected_df <- as.data.frame(fread("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/processing_files/ModelCanalization_BMIWHR/AvgSd_ExpectedDelta_DHAPGS_BMI_PrevPerc.tsv"))
...
o3fa_whr_observed_df <- as.data.frame(fread("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/processing_files/ModelCanalization_BMIWHR/ObservedDelta_DHAPGS_WHR_PrevPerc.tsv"))
o3fa_whr_expected_df <- as.data.frame(fread("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/processing_files/ModelCanalization_BMIWHR/AvgSd_ExpectedDelta_DHAPGS_WHR_PrevPerc.tsv"))
...
#write.table(merge_df,file="/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/tables/ForGreg_DHA_WHR_BMI_departures.tsv",quote=FALSE,sep="\t",row.names=FALSE,col.names = TRUE)
...
ggplot(merge_df, aes(y=whr_departure,x=bmi_departure,color=category)) +
  ...
  labs(x = "BMI delta departure",y="WHR delta departure", title = "Docosahexaenoic acid") + 
  ylim(-5,5) + xlim(-5,5) + 
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/V3_DHA_BMI_DeltaDeparture_WHR_DeltaDeparture_Scatter_plt_Suggestive.pdf",height = 15,width = 15,dpi = 300,limitsize = FALSE)
...
o3fa_bmi_whr_departure_plt <- ggplot(merge_df, aes(reorder(disease,-ratio), ratio,color=category)) + 
  ...
  labs(x="Disease",y="BMI delta departure/WHR delta departure",title="Docosahexaenoic acid") +
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/DHA_BMI_DeltaDeparture_WHR_DeltaDeparture_Suggestive_all_points.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)


o3fa_bmi_whr_departure_plt <- ggplot(merge_df, aes(reorder(disease,-ratio), ratio,color=category)) + 
  scale_color_manual(name="Disease group",values = disease_colors) +
  geom_point(size=5) + theme_bw() + geom_hline(yintercept=0) + coord_cartesian(ylim = c(-45, 25)) + 
  labs(x="Disease",y="BMI delta departure/WHR delta departure",title="Docosahexaenoic acid") +
  theme(axis.text.x = element_text(size = 20,angle = 45, hjust=1), text = element_text(family = "Helvetica",size=20),legend.title=element_text(size=20),legend.text=element_text(size=20),strip.text = element_text(size=20),legend.position="bottom",axis.text.y = element_text(size = 20), axis.title = element_text(size = 20))
o3fa_bmi_whr_departure_plt
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/DHA_BMI_DeltaDeparture_WHR_DeltaDeparture_Suggestive.pdf",height = 15,width = 60,dpi = 300,limitsize = FALSE)
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/MetaboliteXAnthropometric_ModelCanalization_BMIWHRRatio/DHA_BMI_DeltaDeparture_WHR_DeltaDeparture_Scatter_plt_Suggestive.pdf",height = 15,width = 15,dpi = 300,limitsize = FALSE)
