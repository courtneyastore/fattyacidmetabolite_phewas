...
prs_df <- prs_df[c('ID','p23445_zscore')]
prs_df <- prs_df[!is.na(prs_df$p23445_zscore), ]
...
for (disease in disease_lst){
  ...
  output_file_name = paste0("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/LevelsO6FA_BMI_WHR_PrevPerc_Batch/LevelsO6FA_BMI_WHR_",disease_name_file,".pdf")
  ...
  plt <- ggplot(final_df, aes(x=PRS_percentile, y=Prev*100,col = model,fill=model, group = model)) + geom_point(alpha=0.3) + 
    ...
    xlab("Percentile of Omega-6 fatty acid levels") + ylab(disease_name_prev_label) +
