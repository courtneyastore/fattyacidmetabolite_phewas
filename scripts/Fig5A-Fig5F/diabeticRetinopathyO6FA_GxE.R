...
disease_status_df <- disease_status_df[c('ID','Status_250.7')]
...
prs_df <- prs_df[c('ID','PRS_finngen_R5_DM_RETINOPATHY_PRS')]
...
merge_df <- merge_df[merge_df$Status_250.7 != "Excluded", ]
merge_df$disease_status <- ifelse(merge_df$Status_250.7 == "Case", 1, 0)
...
low_high_dha_prev_plt <- ggplot(low_high_DHA_prs_prev_df, aes(x=PRS_percentile, y=Prev*100,col = model,fill=model, group = model)) + geom_point(alpha=0.3) + 
  ...
  xlab("Percentile of PGS-Diabetic retinopathy") + ylab("Prevalence of disease (%)") +
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/PRSxMetabolite/O6FA_diabetic_retinopathy_PGSPrev_plt.pdf",height = 8,width = 8,dpi = 300,limitsize = FALSE)
