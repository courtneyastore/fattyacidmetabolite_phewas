...
disease_status_df <- disease_status_df[c('ID','Status_296.22')]
...
prs_df <- prs_df[c('ID','PRS_ieu-a-1188_PRS')]
...
metabolite_df <- metabolite_df[c('ID','p23450_zscore')]
metabolite_df <- metabolite_df[!is.na(metabolite_df$p23450_zscore), ]
...
MASTER_metabolite_df$p23450_zscore_group <- ifelse(MASTER_metabolite_df$p23450_zscore >= mean(MASTER_metabolite_df$p23450_zscore),1,0) # 1 is HIGH DHA and 0 id LOW DHA
...
model1_prev_perc_df$model <- "Low-Docosahexaenoic acid"
...
model2_prev_perc_df$model <- "High-Docosahexaenoic acid"
...
model_colors <- c("Low-Docosahexaenoic acid" = "#ffaf7a","High-Docosahexaenoic acid" = "#ff781f")
...
ggsave("/Users/courtneyastore/Dropbox (GaTech)/metabolitexenvironment_disease_project/figures/PRSxMetabolite/DHA_MajorDepression_PGSPrev_plt.pdf",height = 8,width = 8,dpi = 300,limitsize = FALSE)
