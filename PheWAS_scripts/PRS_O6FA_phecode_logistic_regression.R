...
out_file = "/storage/home/hcoda1/1/castore3/p-ggibson3-0/PUFA_PheWAS_Frontiers/result_files/11_25_2023_Phecodes_PRS_O6FA_age_agesquare_sex_10PCS_glmresults_"
out_file = paste0(out_file,iter_,".tsv")

#files_lst <- list.files(path=args[1], pattern=".tsv", all.files=TRUE,full.names=TRUE)

master_df <- as.data.frame(fread("/storage/home/hcoda1/1/castore3/p-ggibson3-0/calculate_PRS/PRScs_scores/FinalPRSCalculations/met-d-omega6_PRS.tsv",sep="\t"))
...
covar_df$eid <- paste0(covar_df$eid,"_",covar_df$eid)
#covar_df$eid <- as.character(covar_df$eid)


master_df <- merge(master_df,covar_df,by="eid")
master_df$eid <- as.character(master_df$eid)
